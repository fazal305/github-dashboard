...
function Following() {
  ...
  const { data: following, error, isLoading, refetch } = useGitHubApi(
    (signal) => githubApi.getUserFollowing(activeUser, { page, perPage }, signal),
  ...
  const hasNextPage = (following?.length ?? 0) === perPage;

  return (
    <div>
      <Breadcrumb items={[{ label: 'Dashboard', path: ROUTE_PATHS.DASHBOARD }, { label: 'Following' }]} />
      ...
      <form onSubmit={handleUserSubmit} className="d-flex flex-wrap gd-gap-2 gd-mb-6">
        <label htmlFor="gd-following-user" className="visually-hidden">
          GitHub username
        </label>
        <input
          id="gd-following-user"
          type="text"
          value={userInput}
      ...
      {!activeUser ? (
        ...
        <div className="gd-card">
          <ErrorState
            title={error.status === 404 ? 'User not found' : 'Couldn’t load following'}
        ...
        <div className="gd-card">
          <EmptyState
            ...
            description={page === 1 ? `@${activeUser} isn't following anyone yet.` : "You've reached the end of the list."}
          />
        </div>
      ) : (
        <>
